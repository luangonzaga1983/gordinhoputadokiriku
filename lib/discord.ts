...
async function fetchMessages(channelId: string, limit = 20) {
...
async function deleteMessage(channelId: string, messageId: string) {
  await fetch(`${BASE}/channels/${channelId}/messages/${messageId}`, { method: "DELETE", headers });
}
...
export async function getGameState() {
  const messages = await fetchMessages(GAME_CHANNEL);
...
export async function setGameState(data: unknown, messageId?: string) {
  ...
  if (messageId) return editMessage(GAME_CHANNEL, messageId, content);
  return sendMessage(GAME_CHANNEL, content);
}

export async function getPlayers(): Promise<{ id: string; name: string }[]> {
  const messages = await fetchMessages(PLAYERS_CHANNEL);
  const msg = messages.find((m: { content: string }) => m.content.startsWith(PLAYERS_TAG));
  if (!msg) return [];
  try { return JSON.parse(msg.content.slice(PLAYERS_TAG.length)); } catch { return []; }
}
...
export async function setPlayers(players: { id: string; name: string }[]) {
  ...
  if (existing) await editMessage(PLAYERS_CHANNEL, existing.id, content);
  else await sendMessage(PLAYERS_CHANNEL, content);
}
